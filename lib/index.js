...
Tester.prototype.generate = {
	...
	*/
	integer: function (min, max) {
		min = (min != null ? min : 0);
		if (max == null) {
			var pow = this.exports.helpers.getPower(min) || 1;
			max = Math.pow(10, this.exports.generate.integer(pow, pow + 10));
		}
		var number = min;
		var diff = -min + max;
		if (!diff)
	...
	*/
	...
	/*
		...
		@params: none
	*/
	email: function (name, lastname) {
		name = name || this.exports.generate.name();
		lastname = lastname || this.exports.generate.lastName();
		var ret = name + '.' + lastname + '@' + this.exports.generate.domain();
